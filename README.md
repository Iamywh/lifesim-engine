...
Social maintenance runs at the start of the week with no RNG. It cools strain, applies smooth saturating neglect drift, and updates support-network strength from relationship quality rather than address-book size. Strong trusted relationships contribute most, weak or strained contacts contribute little, and support strength moves gradually toward the target to avoid first-week discontinuities.

Social planning then inspects the already planned M7 routine profile, creates at most one `weekly_social_focus` event, samples up to two known contacts without replacement, exposes `connect:<id>` for each surfaced known contact, may additionally expose `seek_support:<id>` when current need and relationship quality support it, offers at most one new-encounter option, and includes `keep_social_light` when a social event exists. Known-contact options encode visible relationship quality through ordinary M4 fields such as social value, future value, comfort, risk, uncertainty, time, and energy; they do not expose hidden outcome probabilities or RNG rolls. Availability is bounded for ordinary relationships, so close contacts can still be unavailable and weak contacts can occasionally surface.

Known-contact surfacing records candidate weights and weighted-selection draws. New-encounter selection uses data-defined `encounter_weight` values, not responsiveness or supportiveness, and records eligible contacts, candidate weights, total weight, trigger roll, selection roll, and selected contact when triggered. Availability, known-contact sampling, encounter trigger, encounter selection, interaction outcomes, and support outcomes use isolated SHA-256-derived RNG namespaces that include the simulation seed, simulation name, agent ID, week, namespace, and stage provenance, so social mechanics do not perturb event, decision, consequence, passive, employment, or development timelines.
